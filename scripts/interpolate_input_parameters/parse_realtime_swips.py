...
SWIPS_CSV = '/lfs/h1/swpc/wam/noscrub/prod/dcom/solar1_swips_Jan19-22.csv'
SWIPS_START = datetime(2026,1,19,0,0)
SWIPS_END = datetime(2026,1,21,23,59)

MAG_CSV = '/lfs/h1/swpc/wam/noscrub/prod/dcom/solar1_mag_Jan19-22.csv'
MAG_START = datetime(2026,1,19,0,0)
MAG_END = datetime(2026,1,22,23,59)

SWIPS_DATA = {}
with open(SWIPS_CSV) as f:
    reader = csv.reader(f)
    for line in reader:
        if line[0] == 'time':
            continue
        dt = datetime.strptime(line[0].split('.')[0][:-3], '%Y-%m-%d %H:%M')
        try:
            swvel = float(line[-1])
            swden = min([float(line[1]), 300])
        except:
            continue

        if swden > 0 and swvel > 0:
            SWIPS_DATA[dt] = {'swvel': swvel, 'swden': swden}

MAG_DATA = {}
with open(MAG_CSV) as f:
    reader = csv.reader(f)
    for line in reader:
        if line[0] == 'time':
            continue
        ...
        MAG_DATA[dt] = {'swbx': swbx, 'swby': swby, 'swbz': swbz}
...
#            swvel[k] = None
        ...
        for date in self.imf_date_list:
            ...
            try:
                ...
                if fd >= SWIPS_START and fd <= SWIPS_END:
                    if fd in SWIPS_DATA:
                        swvel[date] = SWIPS_DATA[fd]['swvel']
                        swden[date] = SWIPS_DATA[fd]['swden']
                if fd >= MAG_START and fd <= MAG_END:
                    if fd in MAG_DATA:
                        swbx[date] = MAG_DATA[fd]['swbx']
                        swby[date] = MAG_DATA[fd]['swby']
                        swbz[date] = MAG_DATA[fd]['swbz']
                ...
                if fd < SWIPS_START or fd > SWIPS_END:
